...
function NewClaim({ onBack }) {
  ...
  async function handleSubmit(event) {
    ...
    setIsLoading(true);
    setSuccessMessage("جاري إرسال الملفات للذكاء الاصطناعي...");
    ...
    try {
      ...
      const response = await fetch("http://localhost:8002/analyze-claim", {
        method: "POST",
        body: formData,
      });
      ...
      if (!response.ok) {
        throw new Error(data.detail || "فشل تحليل المطالبة");
      }

      console.log("نتيجة الـ AI:", data);
      setSuccessMessage("تم تحليل الملفات بنجاح!");
    } catch (error) {
      console.error(error);
      setSuccessMessage("خطأ بالاتصال بالباك إند، تأكدي أنه شغال.");
    } finally {
      setIsLoading(false);
    }
  }
